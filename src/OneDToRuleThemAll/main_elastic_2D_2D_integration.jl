...
function main_elastic_2D_2D_integration()
    ...
    xbc = [1:2;]; xbc = Int[1, 2, nxdofs-1, nxdofs]
    ybc = [1:2;]; ybc = Int[1, 2, nydofs-1, nydofs]
    ...
    for loadstep in 1:n_loadsteps
        ...
        for modeItr = 2:(n_modes + 1)
            ...
            while true; iterations += 1
                ...
                newXmode = mode_solver(Ux,aX,Uy,aY,E,b,xbc,Val{2}())
                aX[end] = newXmode
                if modeItr == 3
                    return newXmode
                end

                newYmode = mode_solver(Uy,aY,Ux,aX,E,b,ybc,Val{2}())
                aY[end] = newYmode
                if modeItr == 3
                    return newYmode
                end
