...
function DU_solver(d,d_mesh,d_free,d_fe_values,d_mp,Ψ)
    ...
    while true; i += 1
        ...
        g = DU_residual(d_tri,d_mesh,d_free,d_fe_values,d_mp,Ψ)

        if norm(g) < TOL
            # println("Damage field, d, converged in $i iterations, r = $(norm(g)).")
            break
        end

        K = DU_jacobian(d_tri,d_mesh,d_free,d_fe_values,d_mp,Ψ)
